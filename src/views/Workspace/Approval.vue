    ...
    <el-drawer
      ...
      <div style="margin-left: 20px;margin-right: 20px;margin-bottom: 100px;">
        ...
        <div style="margin-top: 10px;text-align: center">
          <el-descriptions title="审批操作" :column="1" style="margin-top: 20px;">
          </el-descriptions>
          <el-button
  ...
  methods: {
      ...
      queryLeaveDetail(index).then(response => {
        if (response.code === 200) {
          ...
          this.approvalProcess = response.data.approvalProcess.map(item => {
            ...
            return { title, description, username, status, reason, completeTime };
          });
    ...
    agree(){
      this.$prompt('请输入审批意见', '提示', {
        ...
        this.drawer1 = false
        this.drawer2 = false
        const agreeData = new FormData()
        agreeData.append("id", this.detailData.id)
        agreeData.append("reason", value)
        agreeApproval(agreeData).then(response => {
          if (response.code === 200) {
            this.$message({
